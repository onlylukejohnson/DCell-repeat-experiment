...
parser.add_argument('--yscale', '-y', help="the y axis max on the graph", required=True, action="store")
...
for filename in os.listdir(args.directory):
	iperf_results.append(parse_iperf(args.directory + '/' +  filename)[:-1])

averages = []	
for timestep_tuple in itertools.izip_longest(*iperf_results, fillvalue =-1):
	sum_time = 0
	count = 0
	for num in timestep_tuple:
		if num != -1:
			sum_time += num
			count += 1
	if count is 0:
		count = 1
	average = float(sum_time) / count
	averages.append(average)

data = [[i+1, d] for i, d in enumerate(averages)]
...
axes = plt.gca()
axes.set_ylim([0, int(args.yscale)])
